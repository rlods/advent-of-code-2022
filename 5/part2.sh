...
P=`cat input.txt | grep "m" | sed -r 's/move ([0-9]+) from ([0-9]+) to ([0-9]+)/\1 \2 \3/g' | awk -v b="$B" -v e="$E" 'BEGIN {print "END {"b} {print "w"$3"=w"$3" substr(w"$2",length(w"$2")-"$1-1");w"$2"=substr(w"$2",1,length(w"$2")-"$1")"} END {print "print "e"}"}'`
awk "$P" < /dev/null
